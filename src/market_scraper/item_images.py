...
for x in cases:
    ...
    # Retry if div_with_image is not found (up to 3 attempts)
    # attempts = 0
    # while not div_with_image and attempts < 3:
    ...
    if div_with_image:
        ...
        if img_tag:
            ...
            if image_response.status_code == 200:
                image_filename = os.path.join(directory, x + '.png')
                with open(image_filename, 'wb') as f:
                    f.write(image_response.content)
                print(x + ' downloaded.')
            else:
                print(x + ' failed to download. Status code:', image_response.status_code)
        else:
            print(x + ' image not found.')
    else:
        print(x + ' URL not found.')
